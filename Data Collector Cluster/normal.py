...
def write_stop(file,diff,misconfig_code,client):
	...
	with open("/home/neha/Data Collector Cluster/Grep/0_0_0/"+file, "a") as dump:
		for i in file_lines:
			dump.write(i)
	ftp.close()
...
def run_script(misconfig_code,misconfig_node):
	...
	channel3.exec_command('cd /usr/local/hadoop/share/hadoop/mapreduce; source ./myenv; ./grep')
	...
	with open("out.txt", "a") as dump:
		dump.write("TIME:"+str(diff)+",CODE:"+str(misconfig_code)+"\n")
		dump.write("STOP\n")
	'''
	write_stop("Master.txt",diff, misconfig_code, client_master)
	write_stop("Slave1.txt",diff, misconfig_code, client_slave1)
	write_stop("Slave2.txt",diff, misconfig_code, client_slave2)
	#os.system("python format.py >> data_0.csv")
